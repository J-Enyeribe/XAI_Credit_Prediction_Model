...
const Predictor: React.FC = () => {
  ...
  const numericFieldRules: { field: keyof LoanFormData; label: string; min?: number; max?: number }[] = [
    { field: 'Age', label: 'Age', min: 18, max: 120 },
    { field: 'Income', label: 'Annual Income', min: 0 },
    { field: 'LoanAmount', label: 'Loan Amount', min: 0 },
    { field: 'CreditScore', label: 'Credit Score', min: 300, max: 850 },
    { field: 'MonthsEmployed', label: 'Months Employed', min: 0 },
    { field: 'NumCreditLines', label: 'Num Credit Lines', min: 0 },
    { field: 'InterestRate', label: 'Interest Rate', min: 0, max: 100 },
    { field: 'LoanTerm', label: 'Loan Term', min: 1 },
    { field: 'DTIRatio', label: 'DTI Ratio', min: 0, max: 1 },
  ];
  ...
  const validateField = (field: keyof LoanFormData, value: string | number): string => {
    const rule = numericFieldRules.find(r => r.field === field);
    if (rule) {
      if (value === '' || value === null || value === undefined) return `${rule.label} is required.`;
      const num = Number(value);
      if (isNaN(num)) return `${rule.label} must be a number.`;
      if (num < 0) return `${rule.label} cannot be negative.`;
      if (rule.min !== undefined && num < rule.min) return `${rule.label} must be at least ${rule.min}.`;
      if (rule.max !== undefined && num > rule.max) return `${rule.label} must be no more than ${rule.max}.`;
      if (field === 'NumCreditLines' && !Number.isInteger(num)) return `${rule.label} must be a whole number.`;
    }
    if (categoricalFields.includes(field) && (value === '' || value === null || value === undefined)) {
  ...
  const handlePredict = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const { field } of numericFieldRules) {
        (sanitizedData[field] as number) = Number(sanitizedData[field]);
      }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const field = name as keyof LoanFormData;
    setFormData(prev => {
  ...
  const inputClass = (field: keyof LoanFormData, extra = '') =>
    `w-full bg-[var(--bg-card)] border rounded p-2 text-sm text-[var(--text-main)] focus:outline-none focus:ring-1 ${
      errors[field] ? 'border-tomato-500 focus:ring-tomato-500' : 'border-[var(--accent)]/30 focus:ring-[var(--accent)]'
    } ${extra}`;
  ...
  return (
    <div className="flex h-screen w-full bg-[var(--bg-dark)] text-[var(--text-main)] overflow-hidden">
      ...
      <div
        className={`${
          isPanelCollapsed ? 'w-16' : 'max-w-[400px] w-[25%]'
        } h-full bg-[var(--bg-dark)] border-r [border:var(--border-thin)] z-10 transition-all duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] relative group flex-shrink-0 flex flex-col`}
      >
        <div className="flex-1 overflow-y-auto custom-scrollbar p-[var(--space-base)]">
        {isPanelCollapsed ? (
          <div className="flex flex-col items-center gap-3 pt-4">
            <button
              onClick={() => navigate('/landing')}
              className="p-2 rounded-lg transition-all active:scale-90 text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-card)]/50"
          ...
          <>
            <button
              onClick={() => navigate('/landing')}
              className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] hover:text-pacific_blue-500 mb-4 transition-colors duration-300"
            >
              <ArrowLeft size={12} />
              ...
              <div className="space-y-[var(--space-tight)]">
                ...
                <div className="grid grid-cols-2 gap-[var(--space-tight)]">
                  <div>
                    ...
                    <input name="Age" type="number" value={formData.Age} onChange={handleInputChange} className={inputClass('Age')} />
                    {errors.Age && <p className="text-tomato-500 text-[10px] mt-1">{errors.Age}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Education Level</label>
                    <select name="Education" value={formData.Education} onChange={handleInputChange} className={inputClass('Education')}>
                    ...
                    {errors.Education && <p className="text-tomato-500 text-[10px] mt-1">{errors.Education}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Marital Status</label>
                    <select name="MaritalStatus" value={formData.MaritalStatus} onChange={handleInputChange} className={inputClass('MaritalStatus')}>
                      <option value="">Select Status</option>
                      <option value="Single">Single</option>
                      <option value="Married">Married</option>
                      <option value="Divorced">Divorced</option>
                    </select>
                    {errors.MaritalStatus && <p className="text-tomato-500 text-[10px] mt-1">{errors.MaritalStatus}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Has Dependents</label>
                    <select name="HasDependents" value={formData.HasDependents} onChange={handleInputChange} className={inputClass('HasDependents')}>
                      <option value="">Select Option</option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </select>
                    {errors.HasDependents && <p className="text-tomato-500 text-[10px] mt-1">{errors.HasDependents}</p>}
                  </div>
                </div>
              </div>
              <div className="space-y-[var(--space-tight)]">
                ...
                <div className="grid grid-cols-2 gap-[var(--space-tight)]">
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Annual Income</label>
                     <input name="Income" type="number" value={formData.Income} onChange={handleInputChange} disabled={formData.EmploymentType === 'Unemployed'} className={`${inputClass('Income')} ${formData.EmploymentType === 'Unemployed' ? 'opacity-50 cursor-not-allowed' : ''}`} />
                    {errors.Income && <p className="text-tomato-500 text-[10px] mt-1">{errors.Income}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Employment Type</label>
                    <select name="EmploymentType" value={formData.EmploymentType} onChange={handleInputChange} className={inputClass('EmploymentType')}>
                    ...
                    {errors.EmploymentType && <p className="text-tomato-500 text-[10px] mt-1">{errors.EmploymentType}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Months Employed</label>
                    <input name="MonthsEmployed" type="number" value={formData.MonthsEmployed} onChange={handleInputChange} className={inputClass('MonthsEmployed')} />
                    {errors.MonthsEmployed && <p className="text-tomato-500 text-[10px] mt-1">{errors.MonthsEmployed}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Credit Score</label>
                    <input name="CreditScore" type="number" value={formData.CreditScore} onChange={handleInputChange} className={inputClass('CreditScore')} />
                    {errors.CreditScore && <p className="text-tomato-500 text-[10px] mt-1">{errors.CreditScore}</p>}
                  </div>
                </div>
              </div>
              <div className="space-y-[var(--space-tight)]">
                ...
                <div className="grid grid-cols-2 gap-[var(--space-tight)]">
                  <div>
                    ...
                    <input name="LoanAmount" type="number" value={formData.LoanAmount} onChange={handleInputChange} className={inputClass('LoanAmount')} />
                    {errors.LoanAmount && <p className="text-tomato-500 text-[10px] mt-1">{errors.LoanAmount}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Loan Term (Months)</label>
                    <input name="LoanTerm" type="number" value={formData.LoanTerm} onChange={handleInputChange} className={inputClass('LoanTerm')} />
                    {errors.LoanTerm && <p className="text-tomato-500 text-[10px] mt-1">{errors.LoanTerm}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Interest Rate (%)</label>
                    <input name="InterestRate" type="number" value={formData.InterestRate} onChange={handleInputChange} className={inputClass('InterestRate')} />
                    {errors.InterestRate && <p className="text-tomato-500 text-[10px] mt-1">{errors.InterestRate}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Debt To Income Ratio</label>
                    <input name="DTIRatio" type="number" step="0.01" value={formData.DTIRatio} onChange={handleInputChange} className={inputClass('DTIRatio')} />
                    {errors.DTIRatio && <p className="text-tomato-500 text-[10px] mt-1">{errors.DTIRatio}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Number of Credit Lines</label>
                    <input name="NumCreditLines" type="number" value={formData.NumCreditLines} onChange={handleInputChange} className={inputClass('NumCreditLines')} />
                    {errors.NumCreditLines && <p className="text-tomato-500 text-[10px] mt-1">{errors.NumCreditLines}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Loan Purpose</label>
                    <select name="LoanPurpose" value={formData.LoanPurpose} onChange={handleInputChange} className={inputClass('LoanPurpose')}>
                    ...
                    {errors.LoanPurpose && <p className="text-tomato-500 text-[10px] mt-1">{errors.LoanPurpose}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Has Mortgage</label>
                    <select name="HasMortgage" value={formData.HasMortgage} onChange={handleInputChange} className={inputClass('HasMortgage')}>
                      <option value="">Select Option</option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </select>
                    {errors.HasMortgage && <p className="text-tomato-500 text-[10px] mt-1">{errors.HasMortgage}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[var(--text-muted)] mb-1">Has Co-Signer</label>
                    <select name="HasCoSigner" value={formData.HasCoSigner} onChange={handleInputChange} className={inputClass('HasCoSigner')}>
                      <option value="">Select Option</option>
                      <option value="Yes">Yes</option>
                      <option value="No">No</option>
                    </select>
                    {errors.HasCoSigner && <p className="text-tomato-500 text-[10px] mt-1">{errors.HasCoSigner}</p>}
                  </div>
                </div>
              </div>
              <button 
                ...
                className="w-full py-3 bg-[var(--accent)] hover:bg-carbon_black-200 text-white font-bold rounded-lg transition-all disabled:opacity-50 active:scale-[0.98]"
              >
      ...
      <div className="relative flex-1 min-w-0 h-full bg-[var(--bg-dark)]">
        <Canvas shadows>
          ...
          <OrbitControls enablePan={false} minDistance={18} maxDistance={18} />
                    ...
                    <div className="space-y-[var(--space-base)] h-full">
                      <div className="flex justify-between items-center">
                        ...
                        <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase shadow-sm ${
                          xaiBundle.prediction.probability <= 0.25
                            ? 'bg-pacific_blue-600 text-white'
                            : xaiBundle.prediction.probability <= 0.5
                              ? 'bg-mustard-500 text-carbon_black-800'
                      ...
                      <div className="space-y-4">
                        <div>
                          <h4 className="text-xs font-bold text-tomato-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                            <span className="w-1 h-1 bg-tomato-500 rounded-full"></span>
                            Risk Increasers
                          </h4>
                          <div className="space-y-[var(--space-tight)]">
                            {(xaiBundle.prediction.explanation?.top_features || [])
                              ...
                              .map((f, i) => (
                                <div key={i} className="flex justify-between items-center p-2.5 bg-tomato-500/5 rounded-lg border [border:var(--border-thin)]">
                                  <span className="text-xs text-[var(--text-main)]">{cleanFeatureName(f.raw_feature)}</span>
                                  <span className="font-mono text-xs font-bold text-tomato-600">
                        ...
                        <div>
                          <h4 className="text-xs font-bold text-pacific_blue-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                            <span className="w-1 h-1 bg-pacific_blue-500 rounded-full"></span>
                            Risk Decreasers
                          </h4>
                          <div className="space-y-[var(--space-tight)]">
                            {(xaiBundle.prediction.explanation?.top_features || [])
                              ...
                              .map((f, i) => (
                                <div key={i} className="flex justify-between items-center p-2.5 bg-pacific_blue-500/5 rounded-lg border [border:var(--border-thin)]">
                                  <span className="text-xs text-[var(--text-main)]">{cleanFeatureName(f.raw_feature)}</span>
                                  <span className="font-mono text-xs font-bold text-pacific_blue-600">
                                    {f.shap_value.toFixed(3)}
                                  </span>
                                </div>
                              ))}
